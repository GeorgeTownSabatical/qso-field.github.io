...
test("framework moves QSO state through contract, route, fabric, chain, and validators", () => {
  ...
  const smartContract: SmartContract<typeof object.state> = {
    mode: "smart",
  ...
  const geniusContract: GeniusContract<typeof object.state> = {
    mode: "genius",
  ...
  const fabric = evaluateFabric(
    [object, geniusResult.object],
    [
      {
  ...
  const morphism = createMorphism<typeof parentFrame.payload>({
    morphismId: "morphism.framework.mark-routed",
    name: "mark routed",
    apply: (state: typeof object.state) => ({ ...state, routedVia: route.selected.routeId }),
  ...
  assert.equal((committed.object.state as Record<string, unknown>).routedVia, "local-fabric");
