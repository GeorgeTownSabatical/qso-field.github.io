...
export function appendChainRecord(chain, event, frame) {
...
export function verifyProofOfCoherence(input) {
    ...
    if (!provenance)
        errors.push("chain contains event without timestamp or qsoId");
    if (!frameHash)
        errors.push("state frame hash does not match canonical content");
    if (!sequence)
        errors.push("chain previous hash does not match expected previous record");
    if (!availability)
        errors.push("state unavailable");
